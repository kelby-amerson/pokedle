<script>
    ...
    async function updateGuess(e) {
        if(e.code === "Enter"){
            ...
            const fetchData = await fetch(`https://pokeapi.co/api/v2/pokemon-species/${guess[guessNumber].value}`);

            const response = await fetchData.json().then((res) => {
...
<Card.Root >
    ...
    <Card.Content >
        <div class="flex flex-row justify-center columns-2 aspect-auto">
